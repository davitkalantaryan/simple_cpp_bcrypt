//
// file:			simple_cpp_bcrypt_core_bcrypt.c
// path:			src/core/simple_cpp_bcrypt_core_bcrypt.c
...
static void blf_enc(struct SBlowfishContext* c, uint32_t* data, uint16_t blocks)
{
	uint32_t* d;
	uint16_t i;
...
static uint32_t Blowfish_stream2word(const uint8_t* data, uint16_t databytes, uint16_t* current)
{
	uint8_t i;
	uint16_t j;
	uint32_t temp;
...
static void NodeBcrypt(const char* key, size_t key_len, const char* salt, char* encrypted)
{
	...
	if (minor <= 'a')
		key_len = (uint8_t)(key_len + (minor >= 'a' ? 1 : 0));
	else
	{
	...
	Blowfish_expandstate(&state, csalt, salt_len,(uint8_t*)key, (uint16_t)key_len);
	for (k = 0; k < rounds; k++) {
...
static void Blowfish_initstate(struct SBlowfishContext* c)
{
	...
	static const struct SBlowfishContext initstate =
	{ {
		{
